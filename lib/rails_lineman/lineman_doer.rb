...
require 'rails_lineman/files'

module RailsLineman
  class LinemanDoer
    def initialize(config)
      gather_assets(config)
      files_setup(config) if config.deployment_method == :copy_files_to_public_folder
  ...
  private
    ...
    def run_npm_install
      puts ">>>> npm install #{system 'npm install'}"
